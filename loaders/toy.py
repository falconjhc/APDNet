...
class ToyLoader(Loader):
    ...
    def __init__(self):
        ...
        self.data_folder = conf['toy']
        self.log = logging.getLogger('toy')
    ...
    def splits(self):
        ...
        splits = [
            ...
            {'validation': list(range(87,100)),  # --> test on p11
             'test': list(range(75,87)),
             'training': list(range(0,75))
             },
    ...
    def load_labelled_data(self, split, split_type, modality='LGE',
        ...
        if not os.path.exists(os.path.join(self.data_folder, npz_prefix + 'toy_image.npz')):
            if modality == 'LGE':
                value_crop = False
            images, masks_mask1, masks_mask2, patient_index,index, slice = \
                self.load_raw_labelled_data(normalise, value_crop)

            # save numpy arrays
            np.savez_compressed(os.path.join(self.data_folder, npz_prefix + 'toy_image'), images)
            np.savez_compressed(os.path.join(self.data_folder, npz_prefix + 'toy_mask1_mask'), masks_mask1)
            np.savez_compressed(os.path.join(self.data_folder, npz_prefix + 'toy_mask2_mask'), masks_mask2)
            np.savez_compressed(os.path.join(self.data_folder, npz_prefix + 'toy_patienet_index'), patient_index)
            np.savez_compressed(os.path.join(self.data_folder, npz_prefix + 'toy_index'),index)
            np.savez_compressed(os.path.join(self.data_folder, npz_prefix + 'toy_slice'), slice)
        # Load data from saved numpy arrays
        else:
            images        = np.load(os.path.join(self.data_folder, npz_prefix + 'toy_image.npz'))['arr_0']
            masks_mask1      = np.load(os.path.join(self.data_folder, npz_prefix + 'toy_mask1_mask.npz'))['arr_0']
            masks_mask2 = np.load(os.path.join(self.data_folder, npz_prefix + 'toy_mask2_mask.npz'))['arr_0']
            patient_index = np.load(os.path.join(self.data_folder, npz_prefix + 'toy_index.npz'))['arr_0']
            index = np.load(os.path.join(self.data_folder, npz_prefix + 'toy_index.npz'))['arr_0']
            slice = np.load(os.path.join(self.data_folder, npz_prefix + 'toy_slice.npz'))['arr_0']


        assert images is not None and masks_mask1 is not None and masks_mask2 is not None \
        ...
        self.log.debug('Loaded compressed toy data of shape: ' + str(images.shape) + ' ' + str(index.shape))

        anato_masks = masks_mask1
        patho_masks = masks_mask2
        anato_mask_names = ['mask1']
        patho_mask_names = ['mask2']
    ...
    def load_unlabelled_data(self, split, split_type, modality='LGE', normalise=True, value_crop=True):
        ...
        :return:                a Data object
        """
        images, index, slice = self.load_unlabelled_images('toy', split, split_type, False, normalise, value_crop,modality=modality)
    ...
    def load_all_data(self, split, split_type, modality='MR', normalise=True, value_crop=True, segmentation_option='-1'):
        ...
        :return:                a Data object
        """
        images, index, slice = self.load_unlabelled_images('toy', split, split_type, True, normalise, value_crop,modality=modality)
        masks = np.zeros(shape=(images.shape[:-1]) + (1,))
        scanner = np.array([modality] * index.shape[0])
        return Data(images, masks, '-1', index, slice, scanner)
    ...
    def load_raw_labelled_data(self, normalise=True, value_crop=True):
        ...
        """
        self.log.debug('Loading mask1-ct data from original location')
        images, masks_mask1, masks_mask2, patient_index, index, slice = [], [], [], [], [], []
        ...
        for patient_counter, patient_i in enumerate(self.volumes):
            ...
            patient_image, patient_mask1, patient_mask2 = [], [], []
            ...
            img_file_list = [file for file in os.listdir(patient_folder)
                             if (not file.startswith('.')) and (not file.find('Img')==-1)]
            mask1_file_list = [file for file in os.listdir(patient_folder)
                            if (not file.startswith('.')) and (not file.find('Mask1') == -1)]
            mask2_file_list = [file for file in os.listdir(patient_folder)
                            if (not file.startswith('.')) and (not file.find('Mask2') == -1)]
            img_file_list.sort()
            mask1_file_list.sort()
            mask2_file_list.sort()
            ...
            for v in range(volume_num):
                ...
                current_mask1_name = mask1_file_list[v]
                current_mask2_name = mask2_file_list[v]
                v_id_from_img = current_img_name.split('_')[1].split('.')[0]
                v_id_from_mask1 = current_mask1_name.split('_')[1].split('.')[0]
                v_id_from_mask2 = current_mask2_name.split('_')[1].split('.')[0]
                assert v_id_from_img==v_id_from_mask1==v_id_from_mask2, 'Mis-Alignment !'
            ...
            for mask1_seg_path in mask1_file_list:
                mask1 = np.array(Image.open(os.path.join(patient_folder,mask1_seg_path)))
                if not (len(np.unique(mask1)) == 1 and np.unique(mask1)[0] == 0):
                    mask1 = mask1 / np.max(mask1)
                patient_mask1.append(np.expand_dims(mask1, axis=-1))
            patient_mask1 = np.concatenate(patient_mask1,axis=-1)
            masks_mask1.append(np.expand_dims(patient_mask1,axis=-1))

            for mask2_seg_path in mask2_file_list:
                mask2 = np.array(Image.open(os.path.join(patient_folder,mask2_seg_path)))
                if not (len(np.unique(mask2)) == 1 and np.unique(mask2)[0] == 0):
                    mask2 = mask2 / np.max(mask2)
                patient_mask2.append(np.expand_dims(mask2, axis=-1))
            patient_mask2 = np.concatenate(patient_mask2,axis=-1)
            masks_mask2.append(np.expand_dims(patient_mask2, axis=-1))


        # move slice axis to the first position
        images = [np.moveaxis(im, 2, 0) for im in images]
        masks_mask1 = [np.moveaxis(m, 2, 0) for m in masks_mask1]
        masks_mask2 = [np.moveaxis(m, 2, 0) for m in masks_mask2]

        # crop images and masks to the same pixel dimensions and concatenate all data
        images_cropped, masks_mask1_cropped = utils.data_utils.crop_same(images, masks_mask1,
                                                                      (self.input_shape[0], self.input_shape[1]))
        _, masks_mask2_cropped = utils.data_utils.crop_same(images, masks_mask2,
                                                         (self.input_shape[0], self.input_shape[1]))


        images_cropped = np.concatenate(images_cropped, axis=0)
        masks_mask2_cropped = np.concatenate(masks_mask2_cropped, axis=0)
        masks_mask1_cropped = np.concatenate(masks_mask1_cropped, axis=0)
        patient_index = np.array(patient_index)
        index = np.array(index)
        slice = np.array(slice)


        return images_cropped, masks_mask1_cropped, masks_mask2_cropped, patient_index, index, slice
    ...
    def load_raw_unlabelled_data(self, include_labelled=True, normalise=True, value_crop=True, modality='LGE'):
        ...
        """
        self.log.debug('Loading unlabelled toy data from original location')
    ...
    def load_unlabelled_images(self, dataset, split, split_type, include_labelled, normalise, value_crop, modality):
        ...
        if os.path.exists(os.path.join(self.data_folder, npz_prefix + 'toy_image.npz')):
            images = \
                np.load(os.path.join(self.data_folder,
                                     npz_prefix + 'toy_image.npz'))['arr_0']
            index  = \
                np.load(os.path.join(self.data_folder,
                                     npz_prefix + 'toy_index.npz'))['arr_0']
            patient_index = \
                np.load(os.path.join(self.data_folder,
                                     npz_prefix + 'toy_patient_index.npz'))['arr_0']
            slice = \
                np.load(os.path.join(self.data_folder,
                                     npz_prefix + 'toy_patient_slice.npz'))['arr_0']
        ...
        # Load from source
            ...
            np.savez_compressed(os.path.join(self.data_folder,
                                             npz_prefix + 'toy_image'), images)
            np.savez_compressed(os.path.join(self.data_folder,
                                             npz_prefix + 'toy_index'), index)
            np.savez_compressed(os.path.join(self.data_folder,
                                             npz_prefix + 'toy_patient_index'), patient_index)
            np.savez_compressed(os.path.join(self.data_folder,
                                             npz_prefix + 'toy_patient_slice'), slice)
