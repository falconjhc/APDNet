...
from costs import make_dice_loss_fnc, weighted_softmax_cross_entropy,  make_mse_loss_func
import logging
...
class BaseNet(object):
    ...
    def compile(self):
        ...
        dice = make_dice_loss_fnc(self.conf.num_masks)

        self.model.compile(optimizer=Adam(lr=self.conf.lr, decay=self.conf.decay), loss=[dice, ce],
                           loss_weights=[1,1])
    def load_data(self):
